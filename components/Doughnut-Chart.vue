<template>
  <div>
    <svg height="5rem" viewBox="0 0 160 160" class="donut-chart">
    ...
    <h3 class="value">{{ sortedValues[0] }}%</h3>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      angleOffset: -90,
      chartData: [],
      colors: ["#e5e9ff", "#cd5c5c", "thistle", "lightgray"],
      cx: 80,
      cy: 80,
      radius: 60,
      sortedValues: [],
  ...
  computed: {
    strokeColor() {
      return (this.colors = [this.color, ...this.colors]);
    },
    ...
    sortInitialValues() {
      this.colors = [this.color, ...this.colors];
